...
use risc0_zkvm::{
    default_executor_from_elf, ExecutorEnv, MemoryImage, Program, ReceiptMetadata,
    SessionReceipt as Receipt, MEM_SIZE, PAGE_SIZE,
};
...
pub fn execute_locally(elf: &[u8], input: Vec<u8>) -> Result<Output> {
    ...
    let mut exec = default_executor_from_elf(env, elf).context("Failed to instantiate executor")?;
    let session = exec
        .run()
...
pub fn prove_alpha(elf: &[u8], input: Vec<u8>) -> Result<Output> {
    ...
    let metadata = receipt
        .segments
        .last()
        .ok_or_else(|| anyhow!("failed to get receipt metadata"))?
        .get_metadata()?;
