...
Instead of verifying the signature or adding the ballot to EVM state, both the `Governor.castVote` and `Governor.castVoteBySig` methods simply log an `event` and add the ballot to a running hash accumulator.
