...
mod tests {
    ...
    fn creat_test_tar_for_upload(tmp_dir: &TempDir, id: u64) -> (u64, PathBuf, String) {
        let mut tar_bytes = vec![];
        let mut tar_builder = tar::Builder::new(&mut tar_bytes);
        let mut header = tar::Header::new_gnu();
        header.set_size(4);
    ...
    fn publish_upload() {
        ...
        let (download_size, payload, sha256) = creat_test_tar_for_upload(&tmp_dir, 1);
        upload_test(
            base_url,
            download_size,
            None, /* platform */
    ...
    fn publish_upload_duplicate_force_true_test(
        ...
        let (download_size, payload, sha256) = creat_test_tar_for_upload(&tmp_dir, 2);
        upload_test(
            base_url,
            download_size,
            b_platform,
