...
#undef b
  ...
  constexpr Fp constPart() const {
    return elems[0];
  }

  constexpr Fp4 operator+=(Fp4 rhs) device {
    for (uint32_t i = 0; i < 4; i++) {
      elems[i] += rhs.elems[i];
    }
    return *this;
  ...
  constexpr Fp4 operator+(Fp4 rhs) device const {
    return Fp4(*this) + rhs;
  }
  ...
  constexpr Fp4 operator-() device const {
    return -Fp4(*this);
  }
  ...
  constexpr Fp4 operator*(Fp rhs) device const {
    return Fp4(*this) * rhs;
  }

  constexpr bool operator==(Fp4 rhs) device const {
    return Fp4(*this) == rhs;
  }
  ...
  constexpr Fp constPart() device const {
    return Fp4(*this).constPart();
  }
};
