...
// cargo run --bin make_poseidon -- rust
...
// This file is automatically generated by: 
// cargo run --bin make_poseidon -- cpp

"#;

use risc0_core::field::baby_bear::BabyBearElem as Elem;
use tempdir::TempDir;
...
const FIELD : usize = 15 * (1 << 27) + 1;  // The baby bear prime
const CELLS : usize = 24;  // 16 for rate, 8 for capacity
const SECURITY : usize = 128;  // The goal security level
const NUM_BITS : usize = 31;  // # of bits in FIELD
const ALPHA : usize = 7;  // Smallest allowed alpha for Baby Bear
...
fn to_elems(input_string : &str) -> Vec<Elem> {
    ...
    for part in input_string.split(",") {
        let tidy_part : String = part.chars().filter(|c| "0123456789abcdef".contains(*c)).collect();
...
fn run_sage() -> String {
    ...
    let binding = TempDir::new("sage_dir").unwrap();
    let tmp_dir = binding.path().to_path_buf();

    // Checkout the 'official' sagemath code
    info!("Checking out hadeshash repo");
    Command::new("git")
        .current_dir(&tmp_dir)
        .arg("clone")
    ...
    let sage_dir = tmp_dir.join("hadeshash");
    ...
    info!("Running some math, please wait about 2 minutes");
    ...
    std::str::from_utf8(&sage_output.stdout).unwrap().to_string()
}
