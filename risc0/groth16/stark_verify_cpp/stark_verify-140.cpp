...
Fr_lt(&expaux[0],&lvar[254],&circuitConstants[277]); // line circom 472
while(Fr_isTrue(&expaux[0])){
{{
Fr_eq(&expaux[0],&lvar[((1 * Fr_toInt(&lvar[254])) + 0)],&circuitConstants[0]); // line circom 473
}}
if(Fr_isTrue(&expaux[0])){
{
Fr_sub(&expaux[2],&circuitConstants[381],&lvar[254]); // line circom 475
Fr_mul(&expaux[1],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[254])) + 254)],&signalValues[mySignalStart + ((1 * Fr_toInt(&expaux[2])) + 0)]); // line circom 475
{{
Fr_eq(&expaux[0],&expaux[1],&circuitConstants[0]); // line circom 475
}}
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template/function " << myTemplateName << " line 475. " <<  "Followed trace of components: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
}
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * (Fr_toInt(&lvar[254]) + 1)) + 254)];
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[254])) + 254)]);
}
}else{
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * (Fr_toInt(&lvar[254]) + 1)) + 254)];
// load src
Fr_sub(&expaux[1],&circuitConstants[381],&lvar[254]); // line circom 479
Fr_mul(&expaux[0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[254])) + 254)],&signalValues[mySignalStart + ((1 * Fr_toInt(&expaux[1])) + 0)]); // line circom 479
// end load src
...
PFrElement aux_dest = &lvar[254];
// load src
Fr_add(&expaux[0],&lvar[254],&circuitConstants[1]); // line circom 472
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[254],&circuitConstants[277]); // line circom 472
