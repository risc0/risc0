...
to_bits_exact_56_create(mySignalStart+2,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 0+ctx_index+1;
}
{
std::string new_cmp_name = "out_bits";
to_bits_exact_57_create(mySignalStart+32,1+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[1] = 1+ctx_index+1;
}
{
std::string new_cmp_name = "vbb";
valid_baby_bear_58_create(mySignalStart+64,2+ctx_index+1,ctx,new_cmp_name,myId);
...
to_bits_exact_56_run(mySubcomponents[cmp_index_ref],ctx);
}
{
...
cmp_index_ref_load = 0;
Fr_mul(&expaux[2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 29],&circuitConstants[276]); // line circom 68
cmp_index_ref_load = 1;
cmp_index_ref_load = 1;
