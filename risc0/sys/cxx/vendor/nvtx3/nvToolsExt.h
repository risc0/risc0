 ...
 * For some platforms or special cases, the user may be required 
 ...
 * correlation of a start/end pair a unique correlation ID is used that is
 ...
 * This allows developers to scope markers, ranges, and resource names to
 * avoid conflicts.
 *
 ...
 *
 * Some functions in this library support associating an integer category 
 ...
 * category. Support for domains has been added in NVTX_VERSION_2 to 
 ...
 * while the type field helps provide a deeper understanding of the identifier as 
 * well as enabling differentiation in cases where handles are generated by different
 ...
 * name, such as some OS thread creation APIs.  This API support resource naming 
 ...
 * text message.
 *
 * \param message     - The message associated with this marker event.
 *
 * \par Example:
 * \code
...
/** \brief Starts a process range.
 *
 * \param message     - The event message associated with this range event.
 *
 * \return The unique ID used to correlate a pair of Start and End events.
 *
 * \remarks Ranges defined by Start/End can overlap.
...
* </ul>
*
* Zeroing the structure sets all the resource attribute types and values
* to the default value.
*
* The version and size field are used by the Tools Extension
* implementation to handle multiple versions of the attributes structure.
*
* It is recommended that the caller use one of the following methods
* to initialize the event attributes structure:
*
* \par Method 1: Initializing nvtxEventAttributes for future compatibility
* \code
