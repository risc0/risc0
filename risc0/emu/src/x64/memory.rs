...
pub const PAGE_SLOT_META_OFFSET: i32 = offset_of!(PageSlot, meta) as i32;
...
impl PageSlot {
    ...
    #[inline]
    pub fn tag(&self) -> u16 {
        self.meta & PAGE_WRITABLE_MASK
    }

    #[inline]
    pub fn writable(&self) -> bool {
        self.meta & PAGE_WRITABLE_FLAG != 0
    }

    #[inline]
    pub fn set(&mut self, ptr: *const u8, tag: u16, writable: bool) {
        self.ptr = ptr;
        self.meta = tag;
        if writable {
            self.meta |= PAGE_WRITABLE_FLAG;
        }
    }
}
...
impl HostMemory {
    ...
    pub fn ensure_page_write_for_segment(
        ...
        slot.set(page_data.as_ptr(), current_tag, true);
