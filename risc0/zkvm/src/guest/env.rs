...
/// Digest of the running list of [Assumptions], generated by the [verify] and [verify_integrity]
/// calls made by the guest.
...
pub fn verify(image_id: Digest, journal: &[u8]) -> Result<(), VerifyError> {
    ...
    // Construct the ReceiptMetadata for this assumption. Use the host provided post_state_digest
    // and fix all fields that are required to have a certain value. This assumption will only be
    // resolvable if there exists a receipt matching this metadata.
...
pub enum VerifyIntegrityError {
    ...
    /// Metadata output was pruned and not equal to the zero hash. It is impossible to determine
    /// whether the assumptions list is empty.
