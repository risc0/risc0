...
pub(crate) fn pprof_path() -> Option<PathBuf> {
    match std::env::var("RISC0_PPROF_OUT") {
        Ok(path) => {
            tracing::debug!("pprof output path set to: {path}");
            Some(PathBuf::from(path))
        }
        Err(std::env::VarError::NotPresent) => None,
        Err(std::env::VarError::NotUnicode(_)) => {
            tracing::warn!("Environment variable RISC0_PPROF_OUT is malformed");
            None
        }
    }
}
...
pub(crate) fn write_pprof_file(profile: &[u8]) -> anyhow::Result<()> {
    ...
    tracing::debug!("writing profile to: {}", path.display());
