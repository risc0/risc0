...
pub trait Prover {
    ...
    /// Prove zkVM execution starting from the specified ELF binary and using the specified
    /// [ProverOpts].
    ///
    ...
    fn prove_with_opts(
        ...
        // TODO(victor): Determine the verifier context from the opts.
        self.prove_with_ctx(env, &VerifierContext::default(), elf, opts)
    }

    // TOOD(victor): Remove prover with ctx in favor of prove with opts?
...
pub struct ProverOpts {
    ...
    // TODO(victor): Should this also be used for the segment prover? What do we set this to when
    // the hash function is sha-256 or blake2b? Maybe this should be called "recursion control
    // IDs".
