...
pub struct ExecutorEnvBuilder<'a> {
    inner: ExecutorEnv<'a>,
    assumption_receipts: Vec<AssumptionReceipt>,
}
...
#[derive(Default)]
pub(crate) struct AssumptionReceipts(pub(crate) HashMap<Assumption, AssumptionReceipt>);
...
impl<'a> ExecutorEnvBuilder<'a> {
    ...
    pub fn build(&mut self) -> Result<ExecutorEnv<'a>> {
        ...
        for assumption_receipt in self.assumption_receipts.iter().cloned() {
            let assumption: Assumption = assumption_receipt.clone().try_into()?;
            inner
                .assumptions
                .borrow_mut()
                .0
                .insert(assumption, assumption_receipt);
        }
    ...
    pub fn add_assumption(
        &mut self,
        assumption_receipt: impl Into<AssumptionReceipt>,
    ) -> &mut Self {
        self.assumption_receipts.push(assumption_receipt.into());
        self
    }
