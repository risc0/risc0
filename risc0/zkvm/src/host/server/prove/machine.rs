...
pub struct MemoryState {
    ...
    pub ram_arg: RamArgument,
    pub bytes_arg: BytesArgument,

    // Argument accumulations for compute_accum and verify_accum phases
    pub arg_accum: BTreeMap<String, ArgumentAccum<BabyBear>>,
}
...
impl MemoryState {
    ...
    fn store_u8(&mut self, addr: u32, value: u8) {
        // tracing::debug!("store_u8: 0x{addr:08X} <= 0x{value:08X}");
        self.ram.store_region_in_page(addr, &[value]);
    }

    #[track_caller]
    fn store_region(&mut self, addr: u32, slice: &[u8]) {
        // tracing::trace!("store_region: 0x{addr:08X} <= {} bytes", slice.len());
    ...
    fn store_u32(&mut self, addr: u32, value: u32) {
        // tracing::debug!("store_u32: 0x{addr:08X} <= 0x{value:08X}");
...
impl CircuitStepHandler<Elem> for MachineContext {
    ...
    fn sort(&mut self, _: &str) {
        self.memory.ram_arg.sort();
        self.memory.bytes_arg.sort();
    }
}
...
impl MachineContext {
    ...
    fn halt(&mut self, cycle: usize, exit_code: Elem, pc: Elem) {
        if !self.is_halted {
            ...
            match exit_code {
                halt::TERMINATE => {
                    tracing::debug!("HALT[{cycle}]> pc: 0x{pc:08x}");
                }
                halt::PAUSE => {
                    tracing::debug!("PAUSE[{cycle}]> pc: 0x{pc:08x}");
                    self.is_flushing = true;
                }
                halt::SPLIT => {
                    tracing::debug!("SPLIT[{cycle}]> pc: 0x{pc:08x}");
                }
    ...
    fn get_major(&mut self, cycle: Elem, pc: Elem) -> Result<Elem> {
        ...
        if let Some(split_insn) = self.split_insn {
            if self.insn_counter == split_insn && !self.is_flushing {
                tracing::debug!("FLUSH[{}]> pc: 0x{pc:08x}", self.insn_counter);
                self.is_flushing = true;
            }
        }
    ...
    fn ram_read(&mut self, cycle: usize, addr: Elem, op: Elem) -> Result<Quad> {
        ...
        if op == MemoryOp::PageIo.as_u32() {
            ...
            tracing::debug!("[{cycle}] ram_read: 0x{addr:08x}, op: {op:?}, entry_addr: 0x{entry_addr:08x}, page_idx: {page_idx}");
    ...
    fn ram_write(&mut self, addr: Elem, data: Quad, op: Elem) -> Result<()> {
        ...
        let addr = addr * WORD_SIZE as u32;
        // tracing::debug!("ram_write> 0x{:08X} <= 0x{:08X}", addr, data);
    ...
    fn arg_read(&mut self, name: &str, outs: &mut [Elem]) {
        match name {
            "ram" => self.memory.ram_arg.read(outs.try_into().unwrap()),
            "bytes" => self.memory.bytes_arg.read(outs.try_into().unwrap()),
    ...
    fn arg_write(&mut self, name: &str, args: &[Elem]) {
        match name {
            "ram" => self.memory.ram_arg.write(args.try_into().unwrap()),
            "bytes" => self.memory.bytes_arg.write(args.try_into().unwrap()),
            _ => panic!("Unknown argument type {name}"),
        }
    }

    fn arg_read_accum(&mut self, name: &str, outs: &mut [Elem]) {
        if let Some(entry) = self.memory.arg_accum.get_mut(name) {
            entry.read(outs)
        } else {
    ...
    fn arg_write_accum(&mut self, name: &str, args: &[Elem]) {
        if let Some(entry) = self.memory.arg_accum.get_mut(name) {
            ...
            accum.write(args);
            self.memory.arg_accum.insert(name.to_string(), accum);
        }
    }
