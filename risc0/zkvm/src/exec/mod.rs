...
/// The LocalExecutor provides an implementation for the execution phase.
///
/// The proving phase uses an execution trace generated by the LocalExecutor.
...
impl<'a> Executor for RemoteExec<'a> {
    fn run(&mut self) -> Result<Session> {
...
impl<'a> LocalExecutor<'a> {
    /// Construct a new [LocalExecutor] from a [MemoryImage] and entry point.
    ///
    /// Before a guest program is proven, the [LocalExecutor] is responsible for
    ...
    /// Construct a new [LocalExecutor] from the ELF binary of the guest program
    /// you want to run and an [ExecutorEnv] containing relevant
    /// environmental configuration details.
    /// # Example
    /// ```
    /// use risc0_zkvm::{serde::to_vec, LocalExecutor, ExecutorEnv, Session};
