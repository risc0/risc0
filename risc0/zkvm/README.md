...
By using the zkVM, a cryptographic [receipt](FlatSessionReceipt) is produced which
anyone can [verify](FlatSessionReceipt::verify) was produced by the zkVM's guest
code. No additional information about the code execution (such as, for example,
the inputs provided) is revealed by publishing the [receipt](FlatSessionReceipt).
