...
template<typename C>
FDEV void ExpandU32<C>::set(CTX, uint32_t val) DEV {
  b0.set(ctx, val & 0xff);
...
template<typename C>
FDEV ValU32<C> ExpandU32<C>::get() DEV {
  return ValU32<C>(b0.get() + b1.get() * 256, b2.get() + b3.get() * 256);
}

template<typename C>
FDEV void ExpandU32<C>::verify(CTX) DEV {
  EQ(b3.get(), topBit.get() * 128 + b3Low7times2.get() * inv(Fp(2)));
}

template<typename C>
FDEV void SplitTotal<C>::set(CTX, uint32_t val) DEV {
...
template<typename C>
FDEV Val<C> SplitTotal<C>::carry() DEV {
  return carryByte.get() + carryBit0.get() * 0x100 + carryBit1.get() * 0x200;
}

template<typename C>
FDEV void SplitTotal<C>::verify(CTX, Val<C> val) DEV {
  EQ(val, out.get()
      + carryByte.get() * 0x10000
      + carryBit0.get() * 0x1000000
...
template<typename C>
FDEV ValU32<C> Multiply<C>::getA() DEV {
  return ax.get();
}

template<typename C>
FDEV ValU32<C> Multiply<C>::getB() DEV {
  return bx.get();
}

template<typename C>
FDEV ValU32<C> Multiply<C>::getOutLow() DEV {
  return ValU32<C>(s0.out.get(), s1.out.get());
}

template<typename C>
FDEV ValU32<C> Multiply<C>::getOutHigh() DEV {
  return ValU32<C>(s2.out.get(), s3Out.get());
}

template<typename C>
FDEV void Multiply<C>::verify(CTX) DEV {
  EQ(s3Out.get() + Val<C>(0x10000) * s3Carry.get(), valS3());
}

template<typename C>
FDEV Val<C> Multiply<C>::valS0() DEV {
...
template<typename C>
FDEV Val<C> Multiply<C>::valS1() DEV {
  return s0.carry() +
    ax.b0.get() * bx.b2.get() +
    ax.b1.get() * bx.b1.get() +
    ax.b2.get() * bx.b0.get() +
...
template<typename C>
FDEV Val<C> Multiply<C>::valS2() DEV {
  Val<C> aNeg = signA.get() * ax.topBit.get();
  Val<C> bNeg = signB.get() * bx.topBit.get();
  return s1.carry() +
    Val<C>(0x20000)
      - (ax.b0.get() + ax.b1.get() * Val<C>(0x100)) * bNeg
      - (bx.b0.get() + bx.b1.get() * Val<C>(0x100)) * aNeg
      + ax.b1.get() * bx.b3.get()
      + ax.b2.get() * bx.b2.get()
      + ax.b3.get() * bx.b1.get()
      + Val<C>(0x100) * (
...
template<typename C>
FDEV Val<C> Multiply<C>::valS3() DEV {
  Val<C> aNeg = signA.get() * ax.topBit.get();
  Val<C> bNeg = signB.get() * bx.topBit.get();
  return s2.carry() +
    Val<C>(0x1fffe)
    - (ax.b2.get() + ax.b3.get() * Val<C>(0x100)) * bNeg
    - (bx.b2.get() + bx.b3.get() * Val<C>(0x100)) * aNeg
    + ax.b3.get() * bx.b3.get();
}
