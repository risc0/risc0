...
#define P2_CALL_ARGUMENT(call) \
  PICUS_ARGUMENT(ctx, \
                 ({ctx.get(call.isFinal), UNPACK_DIGEST(ctx, call.in), \
                  UNPACK_DIGEST(ctx, &(call.data[0])), UNPACK_DIGEST(ctx, &(call.data[8]))}), \
...
template <typename C> FDEV void PageInPageBlock<C>::verify(CTX) DEV {
  #ifdef PICUS
  PAGE_IN_ARGUMENT(ctx, addr.get() * inv(Fp(uint32_t(MPAGE_SIZE_WORDS))) + MEMORY_SIZE_MPAGES, node);
