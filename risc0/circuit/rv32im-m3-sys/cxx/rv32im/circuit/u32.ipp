...
template<typename C>
FDEV void RegU32<C>::set(CTX, uint32_t val) DEV {
  low.set(ctx, val & 0xffff);
  high.set(ctx, val >> 16);
}

template<typename C>
FDEV ValU32<C> RegU32<C>::get() DEV {
  return ValU32<C>(low.get(), high.get());
}

template<typename C>
FDEV void AddU32<C>::set(CTX, uint32_t a, uint32_t b, bool carryIn) DEV {
...
template<typename C>
FDEV ValU32<C> AddU32<C>::get() DEV {
  return ValU32<C>(low.get(), high.get());
}

template<typename C>
FDEV void AddU32<C>::verify(CTX, ValU32<C> a, ValU32<C> b) DEV {
  EQ(low.get() + carryLow.get() * 0x10000, a.low + b.low);
  EQ(high.get() + carryHigh.get() * 0x10000, a.high + b.high + carryLow.get());
}

template<typename C>
FDEV void AddressDecompose<C>::set(CTX, uint32_t val) DEV {
...
template<typename C>
FDEV Val<C> AddressDecompose<C>::wordAddr(ValU32<C> val) DEV {
  return val.high * 0x4000 + mid14.get();
}

template<typename C>
FDEV void AddressDecompose<C>::verify(CTX, ValU32<C> val) DEV {
  EQ(mid14.get() * 4 + low1.get() * 2 + low0.get(), val.low);
}

template<typename C>
FDEV void AddressVerify<C>::set(CTX, uint32_t val, uint32_t mode) DEV {
  highSub.set(ctx, (mode ? 0xffff : 0xbfff) - (val >> 16));
}

template<typename C>
FDEV void AddressVerify<C>::verify(CTX, ValU32<C> val, Val<C> mode) DEV {
  EQ(Val<C>(0xbfff) + mode * Val<C>(0x4000) - val.high, highSub.get());
}

template<typename C>
FDEV void GetSign<C>::set(CTX, uint32_t val) DEV {
  sign.set(ctx, val >> 31);
  signVerify.set(ctx, (val & 0x7fff0000) >> 15);
}

template<typename C>
FDEV void GetSign<C>::verify(CTX, Val<C> inHigh) DEV {
  EQ(inHigh, signVerify.get() * inv(Fp(2)) + sign.get() * 0x8000);
}

template<typename C>
FDEV void NegU32<C>::set(CTX, uint32_t val, uint32_t neg) DEV {
  if (neg) {
...
template<typename C>
FDEV ValU32<C> NegU32<C>::get() DEV {
  return ValU32<C>(outLow.get(), outHigh.get());
}

template<typename C>
FDEV void NegU32<C>::verify(CTX, ValU32<C> in, Val<C> neg) DEV {
  EQZ(neg * (Val<C>(0xffff) - in.low + 1 - Val<C>(0x10000) * carryLow.get() - outLow.get()) +
      (Val<C>(1) - neg) * (outLow.get() - in.low));
  EQZ(neg * (Val<C>(0xffff) - in.high + carryLow.get() - Val<C>(0x10000) * carryHigh.get() - outHigh.get()) +
      (Val<C>(1) - neg) * (outHigh.get() - in.high));
}

template<typename C>
FDEV void AbsU32<C>::set(CTX, uint32_t val, uint32_t isSigned) DEV {
...
template<typename C>
FDEV ValU32<C> AbsU32<C>::getIn() DEV {
  return in.get();
}

template<typename C>
FDEV ValU32<C> AbsU32<C>::getAbs() DEV {
  return ValU32<C>(absLow.get(), absHigh.get());
}

template<typename C>
FDEV void AbsU32<C>::verify(CTX, Val<C> isSigned) DEV {
  EQ(signVerify.get() * inv(Fp(2)) + sign.get() * Val<C>(0x8000), in.high.get());
  EQ(neg.get(), sign.get() * isSigned);
  EQZ(neg.get() * (Val<C>(0xffff) - in.low.get() + 1 - Val<C>(0x10000) * carryLow.get() - absLow.get()) +
      (Val<C>(1) - neg.get()) * (absLow.get() - in.low.get()));
  EQZ(neg.get() * (Val<C>(0xffff) - in.high.get() + carryLow.get() - Val<C>(0x10000) * carryHigh.get() - absHigh.get()) +
