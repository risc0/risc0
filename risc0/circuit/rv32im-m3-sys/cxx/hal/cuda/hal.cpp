...
// query.cu
extern "C" bool cuda_query(void *out, void* data, void* tree, size_t querySize, size_t rows, size_t cols, size_t idx);

// ntt.cu
extern "C" SparkError sppark_init();
extern "C" SparkError sppark_batch_expand(void* out, void* in, uint32_t inPo2, uint32_t expPo2, uint32_t cols);
extern "C" SparkError sppark_batch_NTT(void* d_inout, uint32_t lg_domain_size, uint32_t poly_count);
extern "C" SparkError sppark_batch_iNTT(void* d_inout, uint32_t lg_domain_size, uint32_t poly_count);
extern "C" SparkError sppark_batch_zk_shift(void* d_inout, uint32_t lg_domain_size, uint32_t poly_count);
...
extern "C" bool cuda_mix_poly_coeffs(FpExt* out, const Fp* in, const uint32_t* combos, FpExt mixStart, FpExt mix, uint32_t inputSize, uint32_t count);
...
public:
  ...
  void query(HalArray<Fp> out, HalMatrix<Fp> data, HalArray<Digest> tree, size_t topSize, size_t idx) override {
    ...
    bool ok = cuda_query(toDevPtr(out), toDevPtr(data), toDevPtr(tree), querySize, data.rows(), data.cols(), idx);
    if (!ok) {
  ...
  void batchPolyEval(HalArray<FpExt> out, HalMatrix<Fp> coeffs, HalArray<uint32_t> cols, HalArray<FpExt> xs) override {
    bool ok = cuda_batch_evaluate_any(toDevPtr(out), toDevPtr(coeffs), toDevPtr(cols), toDevPtr(xs), out.size(), coeffs.rows());
    if (!ok) {
  ...
  void combosMix(HalMatrix<FpExt> combos, HalMatrix<Fp> coeffs, HalArray<uint32_t> whichCombo, FpExt cur, FpExt mix) override {
    ...
    bool ok = cuda_mix_poly_coeffs(
        toDevPtr(combos), toDevPtr(coeffs), toDevPtr(whichCombo), cur, mix, coeffs.cols(), combos.rows());
    if (!ok) {
  ...
  void combosPrep(HalMatrix<FpExt> combos, HalArray<FpExt> eval, HalArray<EvalInfo> info, FpExt mix) override {
    ...
    bool ok = cuda_combos_prepare(toDevPtr(combos), toDevPtr(eval), toDevPtr(info), mix, combos.rows(), eval.size());
    if (!ok) {
  ...
  void combosDivide(HalMatrix<FpExt> combos, HalArray<DivideInfo> info) override {
    ...
    for (size_t i = 0; i < info.size(); i++) {
      ...
      LOG(0, "Do divide");
      auto err = supra_poly_divide(poly, combos.rows(), &rem, pInfo[i].z);
      if (err.code != 0) {
  ...
  void friFold(HalMatrix<Fp> out, HalMatrix<Fp> in, FpExt mix) {
  ...
  void computeDataWitness(HalMatrix<Fp> data, HalArray<Fp> globals, HalArray<RowInfo> rows, HalArray<uint32_t> aux, HalArray<uint32_t> tables) override {
    ...
    data_witgen_cuda(toDevPtr(data), toDevPtr(globals), toDevPtr(rows), toDevPtr(aux), toDevPtr(tables), risc0::ROU_FWD[po2], data.rows());
  }
  ...
  void evalCheck(HalMatrix<Fp> check, HalMatrix<Fp> data, HalMatrix<Fp> accum, HalArray<Fp> globals, HalArray<FpExt> accMix, FpExt ecMix) override {
    ...
    eval_check_cuda(toDevPtr(check), toDevPtr(data), toDevPtr(accum), toDevPtr(globals), toDevPtr(accMix), ecMix, risc0::ROU_FWD[po2 + 2], check.rows());
  }
...
private:
  ...
  void copy(IBufferPtr dst, size_t dstOffset, IBufferPtr src, size_t srcOffset, size_t count) override {
    uint8_t* dstPtr = reinterpret_cast<uint8_t*>(static_cast<CudaBuffer*>(dst.get())->devPtr) + dstOffset;
    uint8_t* srcPtr = reinterpret_cast<uint8_t*>(static_cast<CudaBuffer*>(src.get())->devPtr) + srcOffset;
  ...
  void zero(IBufferPtr buf, size_t offset, size_t size) override {
    uint8_t* bufPtr = reinterpret_cast<uint8_t*>(static_cast<CudaBuffer*>(buf.get())->devPtr) + offset;
