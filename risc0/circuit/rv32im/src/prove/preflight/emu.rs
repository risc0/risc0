...
use risc0_circuit_rv32im_sys::{
    DecodeWitness, InstAuipcOptions, InstBranchOptions, InstBranchWitness, InstEcallOptions,
    InstImmOptions, InstImmWitness, InstJalOptions, InstJalWitness, InstJalrOptions,
    InstLoadOptions, InstLuiOptions, InstMretOptions, InstRegOptions, InstRegWitness,
    InstStoreOptions, MakeTableWitness, RegMemReadWitness, RegMemWriteWitness, UnitAddSubWitness,
    UnitBaseWitness, UnitBitWitness, UnitDivWitness, UnitLtWitness, UnitMulWitness,
    UnitShiftWitness,
    opt::{
        AsKind, BitKind, BrKind, DivKind, DivUOptions, MulKind, MulUuOptions, OutKind, ShiftKind,
        UnitKind, UnitOptions,
    },
    visit_rv32im_instr,
};
...
use crate::prove::preflight::{
    constants::{
        MACHINE_REGS_WORD, MEMORY_SIZE_MPAGES, MODE_MACHINE, MPAGE_MASK_WORDS, MPAGE_SIZE_WORDS_PO2,
    },
...
impl Emulator {
    ...
    fn fetch_and_decode(&mut self) -> Option<DecodeWitness> {
        todo!()
    }

    fn trap(&mut self, _reason: &str) -> Result<()> {
        todo!()
    }
    ...
    fn do_inst_reg<Opt: InstRegOptions + UnitOptions>(&mut self, trace: &mut Trace) {
        ...
        tracing::debug!("  rs1Val = {rs1_val}, rs2Val = {rs2_val}");
        tracing::debug!("  rdVal = {rd_val}");
    }

    fn do_inst_imm<Opt: InstImmOptions + UnitOptions>(&mut self, trace: &mut Trace) {
        let ok = Opt::OUT_KIND.unwrap();
        let dinst = trace.get_block(self.dinst.unwrap());
        let (rs1_val, rs1) = self.read_reg(dinst.rs1 as u32, false);
        let fetch = dinst.fetch;
        ...
        tracing::debug!("  rs1Val = {rs1_val}, imm = {imm}");
        tracing::debug!("  rdVal = {rd_val}");
    }

    fn do_inst_load<Opt: InstLoadOptions>(&mut self, _trace: &mut Trace) {
        todo!()
    }

    fn do_inst_store<Opt: InstStoreOptions>(&mut self, _trace: &mut Trace) {
        todo!()
    }

    fn do_inst_branch<Opt: InstBranchOptions + UnitOptions>(&mut self, trace: &mut Trace) {
        ...
        tracing::debug!("  rs1Val = {rs1_val}, rs2Val = {rs2_val}");
        tracing::debug!("  PC = {:x}", self.pc);
    }

    fn do_inst_jal<Opt: InstJalOptions>(&mut self, trace: &mut Trace) {
        let dinst = trace.get_block(self.dinst.unwrap());
        let imm = dinst.imm;
        ...
        self.new_pc = self.pc + imm;
        tracing::debug!("  RD = {dinst_rd}, value = {:x}", rd.value as u32);
        tracing::debug!("  PC = {:x}", self.pc);
    }

    fn do_inst_jalr<Opt: InstJalrOptions>(&mut self, _trace: &mut Trace) {
        todo!()
    }

    fn do_inst_lui<Opt: InstLuiOptions>(&mut self, _trace: &mut Trace) {
        todo!()
    }

    fn do_inst_auipc<Opt: InstAuipcOptions>(&mut self, _trace: &mut Trace) {
        todo!()
    }

    fn do_inst_ecall<Opt: InstEcallOptions>(&mut self, _trace: &mut Trace) {
        todo!()
    }

    fn do_inst_mret<Opt: InstMretOptions>(&mut self, _trace: &mut Trace) {
        todo!()
    }
    ...
    pub fn run(&mut self, trace: &mut Trace, row_count: usize, end_cycle: u32) -> Result<bool> {
        ...
        while !self.is_done(trace, row_count, end_cycle) {
            let decode_wit_index = if self.mode == MODE_MACHINE {
                self.m_inst_cache.get(&self.pc).cloned()
            } else {
                self.us_inst_cache.get(&self.pc).cloned()
            }
            .or_else(|| Some(trace.add_block(self.fetch_and_decode()?)));

            let Some(decode_wit_index) = decode_wit_index else {
                self.trap("Invalid PC or instruction in user mode")?;
                self.cur_cycle += 1;
                self.user_cycles += 1;
                self.pc = self.new_pc;
                continue;
                ...
                ($(($name:ident, $idx:expr, $opcode:expr, $imm_type:expr, $func3:expr, $func7:expr,
                    ...
                    paste::paste! {
                        ...
                        match Opcode::from(decode_wit.opcode) {
                            ...
                            Opcode::Invalid => self.trap("Invalid opcode")?,
                        }
                    }
                };
            }
...
pub fn emulate(
    ...
    tracing::debug!("Cycle = {}", emu.cur_cycle);
    emu.commit(trace)?;
