...
impl<'a, 'b, S: Syscall> Debugger<'a, 'b, S> {
    ...
    fn run_n_steps(&mut self, steps: u32) -> Result<GdbStatus> {
        for _ in 0..steps {
            if let GdbStatus::Event(GdbEvent::ExecHalted) = self.step()? {
                return Ok(GdbStatus::Event(GdbEvent::ExecHalted));
            }
        }
        Ok(GdbStatus::Event(GdbEvent::DoneSteps(steps)))
    }
    ...
    fn step(&mut self) -> Result<GdbStatus> {
        ...
        if let Some(_terminate_state) = self.exec.terminate_state.take() {
            return Ok(Event(ExecHalted));
        }
    ...
    fn run_continue(
        ...
        loop {
            ...
            if let Event(ExecHalted) = self.step()? {
                return Ok(Event(ExecHalted));
            }
        }
    }
        ...
        match target.run(conn) {
            ...
            Ok(GdbStatus::Event(event)) => {
                let stop_event = match event {
                    GdbEvent::ExecHalted => SingleThreadStopReason::Exited(0),
