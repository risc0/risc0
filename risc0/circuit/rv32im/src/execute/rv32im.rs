...
const fn dispatch_index(opcode: u32, func3: u32, func7: u32) -> usize {
    let normalized_func7 = match opcode {
        0b0010011 => match func3 { 0b001 | 0b101 => func7 & 0x7F, _ => 0 },
        0b0000011 | 0b0100011 | 0b1100011 |
        0b0110111 | 0b0010111 | 0b1101111 |
        0b1100111                     => 0,
        _                             => func7 & 0x7F,
    };

    let normalized_func3 = match opcode {
        0b0110111 | 0b0010111 | 0b1101111 => 0,   // LUI, AUIPC, JAL
        0b1100111                         => 0,   // JALR (always funct3=000)
        _                                 => func3 & 0x7,
    };

    ((opcode as usize) << 10) | ((normalized_func3 as usize) << 7)
                           |  (normalized_func7  as usize)
}
