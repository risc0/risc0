...
pub trait SegmentUpdateCallbackFactory {
    ...
    fn with_initial_image(self, initial_image: &MemoryImage)
    -> Result<Self::Callback, Self::Error>;
}
...
pub trait SegmentUpdateCallback {
    ...
    fn on_segment_update(&mut self, update: SegmentUpdate) -> Result<(), Self::Error>;

    /// Method called with a final update if the executor encounters an error.
    #[expect(unused_variables)]
    fn on_execution_error(&mut self, final_update: SegmentUpdate) -> Result<(), Self::Error> {
        Ok(())
    }
}

impl<E, F> SegmentUpdateCallback for F
where
    F: FnMut(SegmentUpdate) -> Result<(), E>,
    E: std::error::Error + Send + Sync + 'static,
{
    type Error = E;

    fn on_segment_update(&mut self, update: SegmentUpdate) -> Result<(), E> {
        self(update)
    }
}
...
impl<T: SegmentUpdateCallback> SegmentUpdateCallbackFactory for T {
    ...
    fn with_initial_image(self, _: &MemoryImage) -> Result<Self::Callback, Infallible> {
        Ok(self)
    }
}
