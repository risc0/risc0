...
constexpr constant size_t kBitWidth = 256;
constexpr constant size_t kByteWidth = kBitWidth / 8;

inline uint64_t this_min(uint64_t lhs, uint64_t rhs) {
  if (rhs < lhs) {
    return rhs;
  }
  return lhs;
...
void
extern_bigintQuotient(device void* ctx, uint32_t cycle, const constant char* extra, Fp args[96], Fp outs[32]) {
