...
load("//bazel/platform:transitions.bzl", "rv32im_linux_transition")
...
def _compile(ctx, cc_toolchain, feature_configuration, sys_copts = []):
    ...
    return cc_common.compile(
        ...
        srcs = ctx.files.srcs,
        user_compile_flags = sys_copts + ctx.attr.copts,
...
def _risc0_cc_binary_impl(ctx, copts = [], linkopts = []):
    cc_toolchain = ctx.toolchains[CC_TOOLCHAIN_TYPE].cc
    feature_configuration = cc_common.configure_features(
        ctx = ctx,
        cc_toolchain = cc_toolchain,
    ...
    (_, compilation_outputs) = _compile(ctx, cc_toolchain, feature_configuration, sys_copts = copts)
    linking_contexts = _get_linking_contexts_from_deps(ctx.attr.deps)
    linking_outputs = cc_common.link(
        ...
        compilation_outputs = compilation_outputs,
        linking_contexts = linking_contexts,
        user_link_flags = linkopts + ctx.attr.linkopts,
...
def _risc0_cc_kernel_binary_impl(ctx):
    copts = [
...
def _risc0_cc_kernel_binary_v1_impl(ctx):
    ...
    linkopts = [
        ...
        # Use a custom linker script for kernel code.
        "-T",
        ctx.file._linker_script_v1.path,
    ]
...
risc0_cc_kernel_binary_v1 = rule(
    implementation = _risc0_cc_kernel_binary_v1_impl,
    attrs = attrs,
    toolchains = [CC_TOOLCHAIN_TYPE],
    fragments = ["cpp"],
    incompatible_use_toolchain_transition = True,
