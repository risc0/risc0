	...
	if (active && payload && payload.length) {
		...
		return (
			<div className="w-[480px] rounded-md bg-black p-2 text-white shadow-2xl">
				<strong className="truncate flex items-center w-full justify-between font-mono text-sm mb-4">
					<span>
						{data.value} {data.unit}
					</span>
					<span className="ml-4 text-muted-foreground text-[10px]">
						({formatNumber(data.rawValue)} Hz)
					</span>
				</strong>
				<div>
					Commit{" "}
...
export function renderGraph({
	...
	const frequencyUnit = determineUnit(
		dataset.map((entry) => entry.bench.value),
	);
