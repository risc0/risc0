...
export async function ApplicationsBenchmarksContent({
	version,
	currentTab,
}: { version: Version; currentTab: string }) {
	const applicationsBenchmarks =
		APPLICATIONS_BENCHMARKS_FILENAMES_TO_TITLES[version];
	const currentApplication = pick(applicationsBenchmarks, [
		`${currentTab}.csv` as keyof typeof applicationsBenchmarks,
	]);
	...
	return (
		<ApplicationsBenchmarksTable
			...
			version={version}
			data={convertCsvToJson(data).filter(
				(element) => element.name ?? element.job_name,
			)}
		/>
	);
}
