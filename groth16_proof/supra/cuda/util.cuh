...
void chacha_generate_random_scalars(field_t* output, const chacha_state state,
...
gpu_ptr_t<fr_t> generate_random_scalars(const gpu_t& gpu, size_t n)
{
    ...
    chacha_generate_random_scalars<8><<<gpu.sm_count(), 1024, 0, gpu>>>
        (&random_scalars[0], state, block_count);

    return random_scalars;
}

__global__ __launch_bounds__(1024)
void coeff_wise_mul_and_sub(fr_t* inout, const fr_t* in1, const fr_t* in2, uint32_t lg_domain_size)
{
...
__global__ __launch_bounds__(1024)
void coeff_wise_mul(fr_t* out, const fr_t* in1, const fr_t* in2, uint32_t lg_domain_size)
{
    size_t tid = threadIdx.x + (size_t)blockIdx.x * blockDim.x;
...
__global__ __launch_bounds__(1024)
void coeff_wise_add(fr_t inout[], const fr_t in[], size_t n)
{
    size_t tid = threadIdx.x + (size_t)blockIdx.x * blockDim.x;
