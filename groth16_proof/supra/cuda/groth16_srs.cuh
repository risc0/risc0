...
private:
    ...
    static inline void check_section_id(const byte* srs_ptr, size_t& cursor, uint32_t expected)
    {
        ...
        if (section_id != expected) {
            throw sppark_error{EINVAL, "SRS file's sections are expected to be in ascending order. expected section %u, got %u: ",
                                       expected, section_id};
        }
    }

    static inline uint64_t read_section_size(const byte* srs_ptr, size_t srs_file_size,
                                             size_t& cursor)
    {
        uint64_t section_size = read_u64(srs_ptr, cursor);

        if (srs_file_size < cursor + section_size) {
            throw sppark_error{EINVAL, "SRS file size/layout mismatch. file size is %zu but expected to be at least %zu: ",
                                       srs_file_size, cursor + section_size};
        }
    ...
    static void read_coeffs(const gpu_t& gpu, slice_t<coeff_t> coeffs, const byte* srs_ptr)
    {
        ...
        const byte (*srs)[coeff_size] =
            reinterpret_cast<decltype(srs)>(srs_ptr);

        gpu.par_map(num_coeffs, batch_size, [&](size_t i) {
            (void)read_coeff(const_cast<coeff_t*>(&coeffs[i]), srs[i]);
        }, num_threads);
    }
#endif
...
public:
    SRS(size_t gpu_id, const char* srs_path) : gpu(select_gpu(gpu_id)), srs_path(srs_path)
    {
        ...
        const byte* srs_ptr = (const byte*)mmap(NULL, srs_file_size, PROT_READ,
        ...
        try {
            ...
            if (num_sections != 10) {
                throw sppark_error{EINVAL, "srs file (\"%s\") does not have the expected number of sections of 10; it has %u sections: ",
                                           srs_file, num_sections};
            }
            ...
            b_g2 = gpu_ptr_t<affine_fp2_t>((affine_fp2_t*)gpu.Dmalloc(data.b_g2.size * sizeof(affine_fp2_t)));
