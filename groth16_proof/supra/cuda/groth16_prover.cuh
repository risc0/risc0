...
public:
    static void LDE_powers(fr_t* inout, bool bitrev, uint32_t lg_dsz, uint32_t lg_blowup,
                           stream_t& stream, const fr_t (*powers)[WINDOW_SIZE])
    {
...
void witness_into_poly(fr_t* out, const coeff_t* coeffs,
                       slice_t<uint32_t> indices, const fr_t* witness)
{
...
public:
    ...
    {
        NTT::Base_dev_ptr(stream, d_poly, lg_domain_size, NTT::InputOutputOrder::NR,
                          NTT::Direction::inverse, NTT::Type::standard);
        LDE_launcher::LDE_powers(d_poly, true, lg_domain_size, 0, stream, partial_shift_powers);
        NTT::Base_dev_ptr(stream, d_poly, lg_domain_size, NTT::InputOutputOrder::RN,
    ...
    msm_results read_fuzzed_results_from_file(const char* file_path)
    {
        ...
        if (file.gcount() != sizeof(msm_results)) {
            throw sppark_error{EINVAL, "fuzzed results file(\"%s\") is not the correct size. expected %zu, got %zu: ",
                                       file_path, sizeof(msm_results), file.gcount()};
        }
    ...
    void init_memory()
    {
        ...
        _partial_shift_powers = gpu_ptr_t<fr_t>((fr_t*)gpu.Dmalloc(WINDOW_NUM * WINDOW_SIZE * sizeof(fr_t)));
        ...
        if ((domain_size & (domain_size - 1)) != 0) {
            throw sppark_error{EINVAL, "size of h is expected to be a power of 2, but is %zu: ", domain_size};
        }
        ...
        partial_shift_powers = reinterpret_cast<decltype(partial_shift_powers)>(&_partial_shift_powers[0]);

        fr_t shift = forward_roots_of_unity[lg_domain_size + 1];
        generate_partial_twiddles<<<WINDOW_SIZE / 32, 32, 0, gpu>>>
            (partial_shift_powers, shift);
        CUDA_OK(cudaGetLastError());
    ...
    groth16_prover(const SRS& srs, const char* preprocessed_coeffs_file,
                   const char* fuzzed_results_file)
    : gpu(srs.gpu), srs(srs), coeffs(gpu, preprocessed_coeffs_file), witness_size(srs.get_a_size()),
      msm_g1(nullptr, srs.get_h_size()), msm_g2(nullptr, srs.get_b_g2_size())
    {
        init_memory();
    ...
    : gpu(srs.gpu), srs(srs), coeffs(gpu, srs.get_coeffs_slice()), witness_size(srs.get_a_size()),
      msm_g1(nullptr, srs.get_h_size()), msm_g2(nullptr, srs.get_b_g2_size())
    {
        init_memory();
        ...
        msm_g1.invoke(fuzzed_results.c, srs.get_c(), srs.get_c_size(),
                      dev_ptr_t<fr_t>{&d_fuzz[witness_size - srs.get_c_size()]}, false);
    ...
    groth16_proof prove(const char* public_file_path, const std::vector<fr_t>& witness)
    {
        if (witness.size() != witness_size) {
            throw sppark_error{EINVAL, "witness size doesn't match the expected witness size. expected %zu, got %zu: ",
        ...
        try {
            ...
            coeff_wise_mul_and_sub<<<gpu.sm_count(), 1024, 0, gpu>>>(&d_a[0], &d_b[0], &d_c[0], lg_domain_size);
            CUDA_OK(cudaGetLastError());
            ...
            msm_g1.invoke(results.c, srs.get_c(), srs.get_c_size(),
                          dev_ptr_t<fr_t>{&d_witness[witness_size - srs.get_c_size()]}, false);
