...
pub fn main() {
    let private_input: PrivateInput = env::read();

    let signature_one_input: String = private_input.signature_one_input; // <ecdsa_sig_address_one + recovery_id>
    let signature_two_input: String = private_input.signature_two_input; // <ecdsa_sig_address_two + recovery_id>
    ...
    let signature_one_bytes =
        hex::decode(sanitized_signature_one_input).expect("Invalid hex string");
    ...
    let recovered_key =
        VerifyingKey::recover_from_digest(signed_message.clone(), &signature_res, recid_res);
    ...
    let log_public_key_hash_one =
        "Recovered Public Key Hash One: ".to_owned() + &pubic_key_one_hash_hex;
    ...
    if actual_leaf_one != pubic_key_one_hash_hex {
        let log_msg = "❌ Recovered public key from signature_one is not a signatory: ".to_owned()
            + &encoded_point_pub_key_one_hex;
        env::log(&log_msg);
        return;
    } else {
        let log_msg =
            "🎉 Recovered public key is a signatory: ".to_owned() + &encoded_point_pub_key_one_hex;
        env::log(&log_msg);
    }
    ...
    let signature_two_bytes =
        hex::decode(sanitized_signature_two_input).expect("Invalid hex string");
    ...
    let log_public_key_hash_two =
        "Recovered Public Key Hash Two: ".to_owned() + &pubic_key_two_hash_hex;
    ...
    if actual_leaf_two != pubic_key_two_hash_hex {
        let log_msg = "❌ Recovered public key from signature_two is not a signatory: ".to_owned()
            + &encoded_point_pub_key_two_hex;
        env::log(&log_msg);
        return;
    } else {
        let log_msg =
    ...
    if pubic_key_one_hash_hex == pubic_key_two_hash_hex {
        let log_msg = "Cannot provide two signatures from the same signatory: ".to_owned()
            + &pubic_key_one_hash_hex;
        env::log(&log_msg);
        return;
    } else {
