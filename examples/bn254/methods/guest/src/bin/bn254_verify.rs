...
fn main() {
    ...
    let inp: (Vec<u8>, Vec<u8>, Vec<u8>, Vec<u8>, Vec<u8>, Vec<u8>, Vec<u8>, Vec<u8>) = env::read();
    ...
    let a_factor = Fr::from_slice(&inp.6).expect("Scalar factor expected");
    let b_factor = Fr::from_slice(&inp.7).expect("Scalar factor expected");
