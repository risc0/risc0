...
fn todo_do_a_thing() -> Receipt { // TODO
    ...
    // let a_g2x = hex::decode("209dd15ebff5d46c4bd888e51a93cf99a7329636c63514396b4a452003a35bf704bf11ca01483bfa8b34b43561848d28905960114c8ac04049af4b6315a41678").expect("valid hex");
    // let a_g2y = hex::decode("2bb8324af6cfc93537a2ad1a445cfd0ca2a71acd7ac41fadbf933c2a51be344d120a2a4cf30c1bf9845f20c6fe39e07ea2cce61f0c9bb048165fe5e4de877550").expect("valid hex");
    ...
    // let b_g2x = hex::decode("198e9393920d483a7260bfb731fb5d25f1aa493335a9e71297e485b7aef312c21800deef121f1e76426a00665e5c4479674322d4f75edadd46debd5cd992f6ed").expect("valid hex");
    // let b_g2y = hex::decode("090689d0585ff075ec9e99ad690c3395bc4b313370b38ef355acdadcd122975b12c85ea5db8c6deb4aab71808dcb408fe3d1e7690c43d37b4ce6cc0166fa7daa").expect("valid hex");
    ...
    let input = vec!(
        (a_g1x, a_g1y, a_g2x_i, a_g2x_r, a_g2y_i, a_g2y_r),
        (b_g1x, b_g1y, b_g2x_i, b_g2x_r, b_g2y_i, b_g2y_r),
    );
    // let input: Vec<([u8; 32], [u8; 32], [u8; 32], [u8; 32], [u8; 32], [u8; 32])> = vec!(
    //     (a_g1x, a_g1y, a_g2x_r, a_g2x_i, a_g2y_r, a_g2y_i),
    //     (b_g1x, b_g1y, b_g2x_r, b_g2x_i, b_g2y_r, b_g2y_i),
    // );
    print!("a_g1y is {:?}\n", input[0].1);
    // TODO
    let mut test_val = U256::from_slice(&input[0].1).unwrap();
    print!("test_val is {:?}\n", test_val);
    let all_ones = U256::from_slice(&hex::decode("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff").unwrap()).unwrap();
    test_val.add(&U256::one(), &all_ones);
    print!("test_val + 1 is {:?}\n", test_val);
    print!("Fq modulus is {:?}\n", Fq::modulus());
    let test_val_fq = Fq::from_slice(&input[0].1).expect("test 2");
    print!("Read as Fq got {:?}\n", test_val_fq);
