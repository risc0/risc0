...
Our ceremony transcript is included in the `zkey` published on ceremony.pse.dev in the "Download ZKey" tab of the [RISC Zero STARK-to-SNARK Prover page][pse-risc0-ceremony]. ([mirror][zkey-mirror]/[compressed version][zkey-compressed]) You can verify it matches the circuit using Circom and snarkjs:
...
3. Download the Powers of Tau (`ptau`) file we used for our ceremony; we used the Hermez rollup with `2^23` powers of tau, which is linked in [the snarkjs README][snarkjs] or [available directly here][powers-of-tau-hez-23] ([mirror][powers-of-tau-hez-23-our-mirror]).
